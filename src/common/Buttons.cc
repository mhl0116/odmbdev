...
namespace emu {
  namespace odmbdev {
     ...
     * older version
     *************************************************************************/
    STEP9bFibers_old::STEP9bFibers_old(Crate * crate, emu::odmbdev::Manager* manager):
      RepeatTextBoxAction(crate, manager, "Step9b Fibers (old)",20){ 
      // Has a "repeat n times" textbox.
    }
    void STEP9bFibers_old::respond(xgi::Input * in, ostringstream & out, const string& textBoxContent_in)
    {
      // how many L1As will we send with each run?
      RepeatTextBoxAction::respond(in, out, textBoxContent_in);
      const unsigned long number_of_l1as_to_send=atoi(textBoxContent.c_str());
      // do a hard reset.
      ccb_->HardReset_crate(); // send a simple hard reset without any sleeps
      usleep(5000000); // sleep a long time after the reset


      // now, run our code.

      // set the fiber mapping check (1 is good, 0 is bad)
      // assume it's good, then mark it as bad in the code if we find it so
      int dcfeb_fiber_mapping[7];
      // then set another array that we'll write to based on the number of packets received by DCFEBS
      int dcfeb_packets_rcv_by_run[7];
      int dcfeb_packets_rcv_total[7];
      float fiber_efficiency = 0.0;  // keep track of our fiber efficiency later

      unsigned int VMEresult;


      // do this for all the DAQMBs
      printf("iterating through DAQMBs...\n");
      for(vector <DAQMB*>::iterator dmb = dmbs_.begin(); dmb != dmbs_.end(); ++dmb){
        // get the slot number from the DAQMB (is this just getting it from the XML file?)
        int slot = (*dmb)->slot();
        printf("dmb in slot %d:\n", slot);

        // slot = slot << 19;  // shift the slot -- not needed with wrapper (JBF)

        printf("shifted slot address: %06x\n", slot);
        fflush(stdout);

        int hardware_version = (*dmb)->GetHardwareVersion();
        printf("hardware version: %d\n", hardware_version);

        if (hardware_version == 2){
          // set our fiber mapping check, as well as our total packet counter (for packet efficiency)
          for (int i = 0; i < 7; i++){
            dcfeb_fiber_mapping[i] = 1;
          }
          for (int i = 0; i < 7; i++){
            dcfeb_packets_rcv_total[i] = 0;
          }

          // loop through DCFEBs, turning them off individually
          for (int dcfeb_number = 1; dcfeb_number <= 7; dcfeb_number++) {
            // reset the packet reception counter for this run
            printf("reset the packet reception counter for this run \n");
	    for (int i = 0; i < 7; i++){
              dcfeb_packets_rcv_by_run[i] = 0;
            }

            fflush(stdout);

            // reprogram DCFEBs (copied and modified from ODMB dev page)
            // W 3010 1    this does the reprogramming.
	    vme_wrapper_->VMEWrite(0x3010,0x1,slot,"Reprogram DCFEBs");
            printf("W %04X %04X \n", (0x3010 & 0xffff), (0x1 & 0xffff));
            usleep(100000);
	    
            fflush(stdout);

            // template for sending VME commands:
            // crate->vmeController()->vme_controller(irdwr,addr,&data,rcv);

            // W 3004 1    global reset
       	    vme_wrapper_->VMEWrite(0x3004,0x1,slot,"Global reset");
            usleep(10000);
            // W 3304 1    set real data and internal trigger
	    vme_wrapper_->VMEWrite(0x3304,0x1,slot,"Set real data and internal triggers");
            usleep(10000);
            // W 401C 180    set 7 dcfeb kill bits to zero
 	    vme_wrapper_->VMEWrite(0x401C,0x180,slot,"Set 7 dcfeb KILL bits to zero");
            usleep(25000);

            fflush(stdout);


            // turn off the desired DCFEB:
            // set the power mask with software
	    printf("Calling (*dmb)->LVDB_map(%d)\n",dcfeb_number - 1);
            int bits_to_shift = (*dmb)->LVDB_map(dcfeb_number - 1); // subtract one since the index starts with 8
            unsigned int mask = 0xff - (0x01 << bits_to_shift);
            printf("dcfeb %d off \n", dcfeb_number);
	    printf("bits to shift: %d, mask: %X\n", bits_to_shift, mask);
	    printf("Calling (*dmb)->lowv_onoff(%X)\n",mask);
            (*dmb)->lowv_onoff(mask); // check to see that this issues mask and load
            usleep(700000);

          
            // do as many times as we said:
            for (unsigned int i = 0; i < number_of_l1as_to_send; i++) {
              // W 3200 4     send L1A
	      vme_wrapper_->VMEWrite(0x3200,0x4,slot,"Send L1A");
              usleep(10000);
            }

            fflush(stdout);

            // for each DCFEB, read the received pulses (should be the number of L1As sent, or zero for the one that's off)
            for (int dcfeb_to_read = 1; dcfeb_to_read <= 7; dcfeb_to_read++) {
              // printf("DCFEB %d: \n", dcfeb_to_read);
              // // W 5010 (DCFEB number)   select DCFEB FIFO
	      // vme_wrapper_->VMEWrite(0x5010,dcfeb_to_read,slot,"Select DCFEB FIFO");
              // usleep(10000);
              // // R 5010 0      read selected DCFEB
	      // VMEresult=vme_wrapper_->VMERead(0x5010,slot,"Read selected DCFEB");
              // usleep(10000);
              // // R 321C 0      read L1A match counter (sent)
	      // VMEresult=vme_wrapper_->VMERead(0x321C,slot,"Read L1A match counter (1)");
              // usleep(10000);
              // // R 327C 0      read L1A match counter (sent)
	      // VMEresult=vme_wrapper_->VMERead(0x327C,slot,"Read L1A match counter (7)");
              // usleep(20000);

              // R 340C 0      read number of received packets
   	      VMEresult=vme_wrapper_->VMERead(0x340C + (dcfeb_to_read * 0x10),slot,"Read n received packets");
              // keep track of this!
              dcfeb_packets_rcv_total[dcfeb_to_read - 1] += VMEresult;
              dcfeb_packets_rcv_by_run[dcfeb_to_read - 1] += VMEresult;
              usleep(10000);
              // R 361C 0      read number of good CRCs
     	      VMEresult=vme_wrapper_->VMERead(0x360C + (dcfeb_to_read * 0x10),slot,"Read n good CRCs");
	      usleep(10000);

              // finally, let's make sure everything is printed
              fflush(stdout);
            }
            // // print the packets received for this run
	    printf("{");
	    for (int i = 0; i < 7; i++){
	      printf("%d, ", dcfeb_packets_rcv_by_run[i]);
	    }
	    printf("}  (#%d should be zero) \n", dcfeb_number);

            // mark the fiber mapping array as bad if it is (we are assuming it's good - we set the array to all ones at the top).
            if (dcfeb_packets_rcv_by_run[dcfeb_number - 1] > ((float)number_of_l1as_to_send / 10.0)){
              dcfeb_fiber_mapping[dcfeb_number - 1] = 0;
            }

            // set the power mask back to all on
            (*dmb)->lowv_onoff(0xff);
          }
          // preface output with some context.
          printf("\n\n\nResults for dmb in slot %d: \n", (*dmb)->slot() );
          // check our fiber mapping, and see if everything is OK
          for (int i = 0; i < 7; i++){
            if (dcfeb_fiber_mapping[i] == 1) {
              printf("DCFEB %d fiber mapping: OK \n", i+1);
            } else {
              printf("DCFEB %d fiber mapping: error -- dcfeb_packets_rcv_by_run=%d \n", i+1,dcfeb_packets_rcv_by_run[i-1]);
            }
          }
          // print total packets received at each to check efficiency
          printf("\nPacket reception efficiency (1.00 is perfect): \n");
          for (int i = 0; i < 7; i++){
            fiber_efficiency = (float)dcfeb_packets_rcv_total[i] / (float)(number_of_l1as_to_send * 6);
	    printf("(Packets Recieved)/(L1As to send * 6) = %f/%f,\n", (float)dcfeb_packets_rcv_total[i], (float)(number_of_l1as_to_send * 6));
            printf("DCFEB %d: %f \n", i+1, fiber_efficiency);
          }
        ...
        } else {    // if it's not a version 2 (ODMB), skip it.
          printf("skipping DMB (not hardware version 2)\n");
          fflush(stdout);
        }
      } // end the for loop cycling through the DMBs
    }
    ...
    STEP9bFibers::STEP9bFibers(Crate * crate, emu::odmbdev::Manager* manager):
      RepeatTextBoxAction(crate, manager, "Step9b Fibers (jack)",20){ 
      // Has a "repeat n times" textbox.
    }
    void STEP9bFibers::respond(xgi::Input * in, ostringstream & out, const string& textBoxContent_in)
    {
      // how many L1As will we send with each run?
      RepeatTextBoxAction::respond(in, out, textBoxContent_in);
      const unsigned long number_of_l1as_to_send=atoi(textBoxContent.c_str());
      if (number_of_l1as_to_send==0) return;

      unsigned int VMEresult;

      // do this for all the DAQMBs
      cout << "iterating through DAQMBs..." << endl;
      for(vector <DAQMB*>::iterator dmb = dmbs_.begin(); dmb != dmbs_.end(); ++dmb){
        // get the slot number from the DAQMB 
	int slot = (*dmb)->slot();
        cout << "dmb in slot " << slot << endl;
        int hardware_version = (*dmb)->GetHardwareVersion();
        cout << "hardware version: " << hardware_version << endl;
        if (hardware_version != 2) {
	  cout << "skipping DMB (not hardware version 2)" << endl;   
	  continue;
	}
	...
	// arrays for storing results of LVMB, copper tests
	vector<vector<int> > copper_mapping;
	bool power_on_correct(true), power_on_others(false);

	vector <CFEB> cfebs = (*dmb)->cfebs();
	// begin loop for powering on DCFEBS
	for(unsigned int dcfeb_dev(0); dcfeb_dev<7; dcfeb_dev++) {
	  unsigned int cmd_kill_d = ~(unsigned int)pow(2,dcfeb_dev);
	  vme_wrapper_->VMEWrite(0x401C,cmd_kill_d,slot,"Set KILL");
	  // turn off the desired DCFEB:
	  int bits_to_shift = (*dmb)->LVDB_map(dcfeb_dev);
	  unsigned int mask = 0x01 << bits_to_shift;
	  (*dmb)->lowv_onoff(mask); // check to see that this issues mask and load
	  usleep(400000);
	  
	  vector<int>copper_mapping_local(7,0);
	  // begin loop to read user code
	  for (CFEBItr cfeb = cfebs.begin(); cfeb != cfebs.end(); ++cfeb ) {
	    unsigned int dcfeb_cop = (*cfeb).number();
	    unsigned int dcfeb_usercode = (*dmb)->febfpgauser(*cfeb);
	    cout << " ********************* " << endl
		 << " FEB" << dcfeb_cop+1 << " : "
		 << " Usercode: " << hex << dcfeb_usercode << endl;
	    ...
	    else if(dcfeb_cop==dcfeb_dev) { // if the one we expect isn't on
	      power_on_correct=false;
	    }
	  } // end loop to read user code
	  // save results in outer vector
	  copper_mapping.push_back(copper_mapping_local);
	...
	if (!power_on_correct&&!power_on_others) {
	  cout << "Power-on test failed--no copper communication." << endl;	  
	  return;
	}
	else if (power_on_others) { // print incorrect mappings
	  for(unsigned int dcfeb_dev(0); dcfeb_dev<7; dcfeb_dev++) { // analyze results of power-on test
	    cout << "dcfeb dev: " << dcfeb_dev+1 << endl;
	    for(unsigned int dcfeb_cop(0); dcfeb_cop<7; dcfeb_cop++) { // check coppers mapping for each DCFEB
	      cout << "dcfeb copper power: " << dcfeb_cop+1
		   << "--" << copper_mapping.at(dcfeb_dev)[dcfeb_cop] 
		   << endl;
	      if (copper_mapping.at(dcfeb_dev)[dcfeb_cop]==1 && dcfeb_cop!=dcfeb_dev) { // incorrect mapping
		cout << "Incorrect mapping--device " << dcfeb_dev+1
		     << " mapped to copper " << dcfeb_cop+1
		     << endl;
	      }
	    } // check coppers loop
	  } // analyze results
	...
	cout << "Power-on test passed for all DCFEBs." << endl;
 

	//store results of fiber test
	vector<vector<int> > nrx_pckt;
	bool fibers_correct(true), fibers_others(false), lost_packets(false);
	// now send L1A matches to one DCFEB at a time
	for (CFEBItr cfeb = cfebs.begin(); cfeb != cfebs.end(); ++cfeb ) { // copper loop	  
	  unsigned int dcfeb_cop = (*cfeb).number();
	  vme_wrapper_->VMEWrite(0x3004,0x1,slot,"Reset counters");
	  usleep(10000);
	  unsigned int cmd_kill_d = ~(unsigned int)pow(2,dcfeb_cop);
	  // kill all but the one DCFEB
	  vme_wrapper_->VMEWrite(0x401C,cmd_kill_d,slot,"Set KILL");
	  for (unsigned int i = 0; i < number_of_l1as_to_send; i++) {
	    // W 3200 4     send L1A
	    vme_wrapper_->VMEWrite(0x3200,0x4,slot,"Send L1A");
	  ...
	  vector<int> nrx_pckt_local;
	  cout << "Copper selected: " << dcfeb_cop+1 << endl;
	  for(unsigned int dcfeb_fib(0); dcfeb_fib<7; dcfeb_fib++) {
	    // vme_wrapper_->VMEWrite(0x5010,dcfeb_fib+1,slot,"Select DCFEB FIFO");
	    unsigned int addr_read_nrx_pckt = 0x340C | (0x00F0&((dcfeb_fib+1)<<4));
	    unsigned int addr_read_l1a_match = 0x320C | (0x00F0&((dcfeb_fib+1)<<4));
	    VMEresult = vme_wrapper_->VMERead(addr_read_l1a_match,slot,"Read L1A matches");
	    cout << "L1A matches for fiber " << dcfeb_fib+1 << ": " << dec << VMEresult << endl;
	    VMEresult = vme_wrapper_->VMERead(addr_read_nrx_pckt,slot,"Read n packets received");
	    cout << "Packets received by fiber " << dcfeb_fib+1 << ": " << dec << VMEresult << endl;
	    nrx_pckt_local.push_back(VMEresult);
	    if (dcfeb_fib==dcfeb_cop) {// the one we sent L1As to
	      if (VMEresult!=number_of_l1as_to_send) {
		lost_packets=true;
		fibers_correct=false;
	      }
	    }
	    else if (VMEresult>0) { //packets where we don't expect them
	      fibers_others=true;
	    }
	  } // end fiber loop
	...
	// Print results of fiber test
	if (fibers_others) {
	  cout << "Incorrect fiber mapping:" << endl; 
	  for (CFEBItr cfeb = cfebs.begin(); cfeb != cfebs.end(); ++cfeb ) { // copper loop	  
	    unsigned int dcfeb_cop = (*cfeb).number();	  
	    for (unsigned int dcfeb_fib(0); dcfeb_fib<7; dcfeb_fib++) { 
	      int nrx(nrx_pckt[dcfeb_cop].at(dcfeb_fib));
	      if (nrx_pckt[dcfeb_cop].at(dcfeb_fib)>0) { // read unexpected packets
		cout << "Device " << dcfeb_cop+1
		     << " mapped to fiber " << dcfeb_fib+1
		     << "(" << nrx << " packets received--efficiency = " 
		     << nrx/(float)number_of_l1as_to_send
		     << endl;	  
	      } // read unexpected packets
	    } // fib loop
	  } // dev loop
	}
	else {
	  cout << "Fiber mapping is correct:" << endl;
	  for (unsigned int dcfeb_fib(0); dcfeb_fib<7; dcfeb_fib++) {
	    int nrx(nrx_pckt[dcfeb_fib].at(dcfeb_fib)); 
	    cout << "Fiber " << dcfeb_fib+1 
		 << " received " << nrx
